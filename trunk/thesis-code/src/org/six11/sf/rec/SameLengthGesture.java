...
public class SameLengthGesture extends RecognizedItemTemplate {
  ...
  public void create(RecognizedItem item, SketchBook model) {
    bug("creating same length constraint.");
    Segment s1 = item.getSegmentTarget(TARGET_A);
    Segment s2 = item.getSegmentTarget(TARGET_B);
    NumericValue dist = new NumericValue((s1.length() + s2.length()) / 2.0);
    model.getConstraints().addConstraint(new DistanceConstraint(s1.getP1(), s1.getP2(), dist));
    model.getConstraints().addConstraint(new DistanceConstraint(s2.getP1(), s2.getP2(), dist));
